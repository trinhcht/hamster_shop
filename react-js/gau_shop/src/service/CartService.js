...
export const setQuantity = async (idCus, idPro, quantity, token) => {
    const headers = {
        Authorization: "Bearer " + token,
    };
    try {
        await axios.get(`http://localhost:8080/api/user/setQuantity?idCus=${idCus}&&idProduct=${idPro}&&quantity=${quantity}`, {headers})
    } catch (e) {
        console.log(e)
    }
}
