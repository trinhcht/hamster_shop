...
import "./cart.css"
import {deleteCart, findAllCart, pay, saveCart, setQuantity} from "../../service/CartService";
...
export function Cart() {
    ...
    const handlePlus = async (idCart, idProduct, idCustomer) => {
        ...
        if (result < cart.product.quantity) {
            ...
            await setQuantity(idCustomer, idProduct, result, token)
        }
    }
    ...
    const handleMinus = async (idCart, idProduct, idCustomer) => {
        const cart = listCart.find(value => value.product.id === idProduct)
        let result = document.getElementById(idCart).value
        if (result < cart.product.quantity && result > 1) {
            const results = +document.getElementById("abc").value
            document.getElementById(idCart).value = --result;
            document.getElementById(idCart + 1).innerHTML = VND.format(cart.product.price * result)
            document.getElementById("tongtien").innerHTML = VND.format(results - cart.product.price)
        ...
        <>
            <Header/>
            {
                listCart && listCart.length === 0 ? <>
                                ...
                                <div className="card">
                                    <div className="card-body cart">
                                        <div className="col-sm-12 empty-cart-cls text-center">
                                            ...
                                            <h4>Vui lòng chọn sản phẩm khác</h4>
                                            <a
                                                style={{backgroundColor: "#faaee5", borderColor: "#663399"}}
                                                href="/"
                    ...
                    <div className="bodywrap" style={{marginTop: "9rem"}}>
                        <section className="bread-crumb"
                                 style={{
                                     backgroundImage: `url(${Banner})`,
                                 }}
                        >
                            <div className="container">
                                <div className="title-bread-crumb" style={{color: "rgb(0,0,255)"}}>Giỏ hàng</div>
                            </div>
                        </section>
                                                    ...
                                                    <div className="bill-field">
                                                        ...
                                                        <div className="form-group">
                                                            ...
                                                            <textarea
                                                                ...
                                                                defaultValue={"280 Hoàng Diệu, Đà Nẵng"}
                                                            />
                                                        </div>
                                                    </div>
                                                </div>
